  ...
  return (
    <>
      ...
      <div style={{ display:'flex', flexDirection:'column', height:'100vh', background:'#fff', fontFamily:'Nunito, sans-serif', overflow:'hidden' }}>
        ...
        {/* Main */}
        <main style={{ flex:1, position:'relative', display:'flex', alignItems:'center', padding:'2rem 4rem', overflow:'hidden', minHeight:0 }}>

          {/* === SCATTERED ICONS ALL OVER === */}

          {/* Top-left area */}
          <div className="icon1" style={{ position:'absolute', top:'8%', left:'5%', zIndex:4 }}>
            <Sparkles size={22} fill="#4FC3C3" strokeWidth={0} />
          </div>
          <div className="icon2" style={{ position:'absolute', top:'20%', left:'20%', zIndex:4 }}>
            <Camera size={38} strokeWidth={1.5} color="#ccc" />
          </div>

          {/* Top-center */}
          <div className="icon3" style={{ position:'absolute', top:'5%', left:'45%', zIndex:4 }}>
            <Camera size={60} strokeWidth={1.5} color="#bbb" />
          </div>
          <div className="icon4" style={{ position:'absolute', top:'12%', left:'60%', zIndex:4 }}>
            <Sparkles size={18} fill="#FF9A1F" strokeWidth={0} />
          </div>

          {/* Top-right */}
          <div className="icon5" style={{ position:'absolute', top:'6%', right:'8%', zIndex:4 }}>
            <Sparkles size={28} fill="#4FC3C3" strokeWidth={0} />
          </div>
          <div className="icon6" style={{ position:'absolute', top:'25%', right:'15%', zIndex:4 }}>
            <Camera size={44} strokeWidth={1.5} color="#ddd" />
          </div>

          {/* Mid-left */}
          <div className="icon2" style={{ position:'absolute', top:'50%', left:'8%', zIndex:4 }}>
            <Sparkles size={20} fill="#FFD84D" strokeWidth={0} />
          </div>

          {/* Bottom-left */}
          <div className="icon3" style={{ position:'absolute', bottom:'18%', left:'15%', zIndex:4 }}>
            <Camera size={32} strokeWidth={1.5} color="#ccc" />
          </div>
          <div className="icon1" style={{ position:'absolute', bottom:'10%', left:'40%', zIndex:4 }}>
            <Sparkles size={16} fill="#FF9A1F" strokeWidth={0} />
          </div>

          {/* Bottom-center / right */}
          <div className="icon4" style={{ position:'absolute', bottom:'15%', left:'55%', zIndex:4 }}>
            <Sparkles size={24} fill="#4FC3C3" strokeWidth={0} />
          </div>

          {/* === FLOATING DOTS === */}
          <div className="dot1" style={{ position:'absolute', top:'18%', left:'38%', width:14, height:14, borderRadius:'50%', background:'#FF9A1F', zIndex:4 }} />
          <div className="dot2" style={{ position:'absolute', bottom:'28%', left:'28%', width:10, height:10, borderRadius:'50%', background:'#4FC3C3', zIndex:4 }} />
          <div className="dot3" style={{ position:'absolute', top:'60%', left:'50%', width:12, height:12, borderRadius:'50%', background:'#FFD84D', zIndex:4 }} />

          {/* === LEFT TEXT === */}
          <div style={{ position:'relative', zIndex:4, maxWidth:520, flexShrink:0 }}>
            <h1 style={{ fontFamily:'Baloo 2', fontWeight:800, fontSize:'clamp(3rem, 5vw, 5rem)', lineHeight:1.05, color:'#111', marginBottom:'1.5rem' }}>
              Take photos<br />
              with{' '}
              <span className="pengu-word">PENGU</span>
            </h1>
          ...
          {/* === PENGU RIGHT === */}
          <div style={{ position:'absolute', right:0, bottom:0, top:0, zIndex:2, display:'flex', alignItems:'flex-end', justifyContent:'flex-end', width:'55%', pointerEvents:'none' }}>
            <div style={{ position:'absolute', right:'5%', bottom:'-10%', width:'70%', paddingBottom:'70%', borderRadius:'50%', background:'#FFD84D', opacity:0.4, zIndex:0 }} />
            <img
              src={pengu}
              alt="Pengu"
              style={{ position:'relative', zIndex:1, height:'100%', maxHeight:'100%', objectFit:'contain', objectPosition:'bottom right', display:'block' }}
            />
          </div>
        ...
        <footer style={{ position:'absolute', bottom:'0.75rem', left:'2rem', fontSize:'0.8rem', color:'#aaa', lineHeight:1.8, zIndex:5 }}>
